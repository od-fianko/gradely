...
import {
  registerSchema, type RegisterSchema, LEVELS,
...
export function RegisterForm() {
  ...
  // ── Step 1: role, name, email, level/program, password ──────────────────────
  const form = useForm<RegisterSchema>({
    resolver: zodResolver(registerSchema),
    defaultValues: { role: "STUDENT", name: "", email: "", level: 100, program: "", password: "", confirmPassword: "" },
  });
  ...
  const submitForm = async (data: RegisterSchema) => {
    ...
    if (!res.ok) { setServerError(json.error ?? "Could not create account"); return; }
    setEmail(data.email);
    setStep("otp");
  };
  ...
  if (step === "otp") {
    return (
      <div>
        ...
        <p className="mb-[22px] mt-1 text-[13.5px] text-zinc-500">Enter the code we just sent you</p>
  ...
  return (
    <div>
      <h1 className="text-[19px] font-bold text-zinc-900">Create your account</h1>
      <p className="mb-[22px] mt-1 text-[13.5px] text-zinc-500">Start setting assignments in minutes</p>
        ...
        <form onSubmit={form.handleSubmit(submitForm)} className="space-y-4">
            ...
            <FormItem>
              ...
              <FormControl>
                <input type="email" placeholder="you@university.edu" autoComplete="email"
                  disabled={form.formState.isSubmitting} className={inputClass} {...field} />
              </FormControl>
              <FormMessage />
            </FormItem>
          )} />
          ...
          <FormField control={form.control} name="name" render={({ field }) => (
            <FormItem>
              ...
              <FormControl><input placeholder="Ama Mensah" disabled={form.formState.isSubmitting} className={inputClass} {...field} /></FormControl>
              <FormMessage />
            </FormItem>
          )} />
            ...
            {role === "STUDENT" && (
              <FormField control={form.control} name="level" render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <select disabled={form.formState.isSubmitting} className={inputClass}
            ...
            <FormField control={form.control} name="program" render={({ field }) => (
              <FormItem>
                <FormLabel className={labelClass}>
                  {role === "LECTURER" ? "Department" : "Program"} <span className="font-normal text-zinc-400">(optional)</span>
                </FormLabel>
                <FormControl><input placeholder="Computer Science" disabled={form.formState.isSubmitting} className={inputClass} {...field} /></FormControl>
                <FormMessage />
              </FormItem>
            )} />
          </div>
              ...
              <FormControl>
                <div className="relative">
                  <input type={showPassword ? "text" : "password"} placeholder="Min 8 chars, 1 uppercase, 1 number"
                    disabled={form.formState.isSubmitting} className={`${inputClass} pr-10`} {...field} />
